...
IQR=load('L:\map_matrix\severity_model_wise\observation_parameters\IQR.txt');
...
for ssp=1:3
    ...
    for model=1
        ...
        pp=[pp,nanmean(pdsi(3:end,4:end),2)];
...
%                 int=quantile(data(data(:,1)>=y1&data(:,1)<=y2&data(:,2)>=nov|data(:,1)>=y1&data(:,1)<=y2&data(:,2)<=apr,4),0.75)-quantile(data(data(:,1)>=y1&data(:,1)<=y2&data(:,2)>=nov|data(:,1)>=y1&data(:,1)<=y2&data(:,2)<=apr,4),0.25);
            ...
            %%  % %%%%%%%%%%%%%%%%%%                                      Only drought                          %%%%%%%%%%%%%%%%%%%%%%%%
            dr=find(DATA(:,4)<drth);
            final=DATA(:,1:4);final(:,5:7)=[data1f(:,4),(data1f(:,4)-IQR(ii,2))./IQR(ii,1),data1f(:,5)];
            final(:,8)=DATA(:,5);
            ...
            output{ii,1}=round(final,2);
            disp([ssp,model,ii]);            
        end
        %% save the data
            save(['L:\mastermatrix_matfiles_ssp_output_folder_10thmay2021\95new\','master_matrix95_',num2str(ssp),'_',num2str(model),'.mat'],'output','-v7.3');
        toc
    end
end

% save('L:\Master_matrix\obsdata\final_master_matrix95.mat','output','-v7.3');

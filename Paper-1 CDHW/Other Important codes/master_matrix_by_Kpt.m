...
for ssp=3
    ...
    for model=1:2
        ...
        %% tmax data
        %%%%  txd=load('L:\Data_preprocess\MainDataFile\Tmax_1982_2100\ssp126\tasmax_day_ACCESS-CM2_ssp126.txt'); %% in farenheit
        % xx=find(tmax_data(:,1)>=2020);
        % txd=tmax_data;
        % txd(xx,:)=[]; %% only considered the dataset 1982-2020
        ...
        tic
        %%% pdsi=load('L:\sc_pdsi_daily_1982_2100\ssp126\ACCESS_CM2_daily_pdsi_1982_2100.txt');
        % xx=find(pdsi(:,1)>=2020);
        % pdsi_his=pdsi;
        % pdsi_his(xx,:)=[];

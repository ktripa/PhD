...
%% obs data
data=importdata(['L:\map_matrix\severity_model_wise\observation_parameters\CDHW_obsdata_values90.mat']);
...
final{1,1}(1:38,2:end)=final{1,1}(1:38,2:end)/1.29;
...
for i=1:4
    final{3,1}(1:29,1+i)=final{3,1}(1:29,1+i).*linspace(1.9,3.5,29)';
end
%% axes
% close all;
fig=figure(1);
...
ylabel('Severity');
set(ax(3),'YTick',[0 50 100 150 ],'YLim',[0 200])

saveas(fig,'L:\codes_paper1\Plot1_data\final_figures\area_plot_90.png')
savefig(fig,'L:\codes_paper1\Plot1_data\final_figures\area_plot_90.fig')